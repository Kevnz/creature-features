...
module.exports = function (config) {
  ...
  featuresFile = `${endWith(settings.location, '/') ? settings.location : settings.location + '/'}${env}.json`;
  baseFeatures = `${endWith(settings.location, '/') ? settings.location : settings.location + '/'}default.json`;
  ...
  const requiredFeatures = featuresFiles.filter((f) => exists(f)).map(function (file) {
    return require(file);
  });
  ...
  const featureProxy = new Proxy(feats, {
    get: (obj, prop) => {
      ...
      if (obj[prop].range !== undefined) {
        //validate
        const total = obj[prop].range.reduce((previous, current) => (previous + current), 0);
        if (total !== 100) {
